...
func nationInfo(w http.ResponseWriter, r *http.Request, dbPool *pgxpool.Pool) {
	...
	err := dbPool.QueryRow(r.Context(), "SELECT account_name, region_name, cash_in_hand, cash_in_escrow FROM accounts, nation_permissions WHERE nation_name = $1 AND accounts.account_name = region_accounts.nation_name;", requedNat).Scan(&returnHello.NationName, &returnHello.Region, &returnHello.CashInHand, &returnHello.CashInEscrow)
